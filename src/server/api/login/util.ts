...
export function generateOtp(req: Express.Request, res: Express.Response) {
  ...
  bcrypt.hash(otp, saltRounds, (error, hashedOtp) => {
    ...
    setOtpForEmail(email, otpObject)
      .then(() => {
...
export function verifyOtp(req: Express.Request, res: Express.Response) {
  ...
  getOtpForEmail(email)
    .then((retrievedOtp) => {
      ...
      bcrypt
        .compare(otp, retrievedOtp.hashedOtp)
        .then((compareResponse) => {
          ...
          if (!compareResponse) {
            ...
            if (retrievedOtp.retries > 0) {
              setOtpForEmail(email, modifiedOtp).catch((error) => {

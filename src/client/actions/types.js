/* Login actions */
export const GET_OTP_EMAIL_SUCCESS = 'GET_OTP_EMAIL_SUCCESS'
export const GET_OTP_EMAIL_PENDING = 'GET_OTP_EMAIL_PENDING'
export const GET_OTP_EMAIL_ERROR = 'GET_OTP_EMAIL_ERROR'
export const VERIFY_OTP_ERROR = 'VERIFY_OTP_ERROR'
export const VERIFY_OTP_PENDING = 'VERIFY_OTP_PENDING'
export const RESEND_OTP_PENDING = 'RESEND_OTP_PENDING'
export const RESEND_OTP_DISABLED = 'RESEND_OTP_DISABLED'
export const SET_OTP = 'SET_OTP'
export const SET_EMAIL = 'SET_EMAIL'
export const SET_EMAIL_VALIDATOR = 'SET_EMAIL_VALIDATOR'
export const IS_LOGGED_IN_SUCCESS = 'IS_LOGGED_IN_SUCCESS'
export const IS_LOGGED_OUT = 'IS_LOGGED_OUT'

/* User actions */
export const CREATE_URL_SUCCESS = 'CREATE_URL_SUCCESS'
export const GET_URLS_FOR_USER_SUCCESS = 'GET_URLS_FOR_USER_SUCCESS'
export const OPEN_CREATE_URL_MODAL = 'OPEN_CREATE_URL_MODAL'
export const CLOSE_CREATE_URL_MODAL = 'CLOSE_CREATE_URL_MODAL'
export const SET_SHORT_URL = 'SET_SHORT_URL'
export const SET_LONG_URL = 'SET_LONG_URL'
export const SET_EDITED_LONG_URL = 'SET_EDITED_LONG_URL'
export const SET_RANDOM_SHORT_URL = 'SET_RANDOM_SHORT_URL'
export const RESET_USER_STATE = 'RESET_USER_STATE'
export const TOGGLE_URL_STATE_SUCCESS = 'TOGGLE_URL_STATE_SUCCESS'
export const OPEN_QR_CODE = 'OPEN_QR_CODE'
export const CLOSE_QR_CODE = 'CLOSE_QR_CODE'
export const OPEN_OWNERSHIP_MODAL = 'OPEN_OWNERSHIP_MODAL'
export const CLOSE_OWNERSHIP_MODAL = 'CLOSE_OWNERSHIP_MODAL'
export const SET_NEW_OWNER = 'SET_NEW_OWNER'
export const EDIT_LONG_URL = 'EDIT_LONG_URL'
export const CANCEL_EDIT_LONG_URL = 'CANCEL_EDIT_LONG_URL'
export const SET_URL_TABLE_CONFIG = 'SET_URL_TABLE_CONFIG'
export const UPDATE_URL_COUNT = 'UPDATE_URL_COUNT'

/* Common actions */
export const CLOSE_SNACKBAR = 'CLOSE_SNACKBAR'
export const SET_ERROR_MESSAGE = 'SET_ERROR_MESSAGE'
export const SET_INFO_MESSAGE = 'SET_INFO_MESSAGE'
export const LOAD_STATS = 'LOAD_STATS'

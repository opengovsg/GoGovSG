...
|         FF_ONE_GOV_SG_LOGIN        |    No    | Boolean, feature flag for enabling login with one.gov.sg. Defaults to false. The four `ONE_GOV_SG_*` variables below are required when true                                                    |
|         ONE_GOV_SG_ISSUER          |    No    | one.gov.sg OIDC issuer, e.g. `https://one.gov.sg/api/auth`                                                                                                                                     |
|        ONE_GOV_SG_CLIENT_ID        |    No    | Client ID registered with one.gov.sg                                                                                                                                                           |
|      ONE_GOV_SG_REDIRECT_URI       |    No    | Registered callback URL, `<origin>/api/sso/callback`                                                                                                                                           |
|       ONE_GOV_SG_PRIVATE_KEY       |    No    | RS256 private key for `private_key_jwt`, PKCS#8 PEM. Newlines may be stored as literal `\n`                                                                                                    |
